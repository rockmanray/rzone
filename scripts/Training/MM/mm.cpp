...
void CGameCollection::MM(int Feature)
{
 ...
 for (int i = vgame.size(); --i >= 0;)
 {
  ...
  {
   const CTeam &team = vgame[i].Winner;
...
// Write database
/////////////////////////////////////////////////////////////////////////////
void WriteDB(const CGameCollection &gcol)
{
 double *dGamma = new double[gcol.vGamma.size()];
 
 for (unsigned i = 0; i < gcol.vGamma.size(); i++) { dGamma[i] = gcol.vGamma[i]; }
 
 std::ofstream fout("MM.db",std::ios::out|std::ios::binary);
 fout.write(reinterpret_cast<char*>(dGamma), sizeof(double)*gcol.vGamma.size());
 fout.close();
}
